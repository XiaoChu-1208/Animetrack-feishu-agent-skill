...
| Aily 没有 Open API 权限 | API 调用返回权限错误时**立刻停下来**告诉用户具体缺哪个权限标识符（**原样输出，不要翻译**，比如 `base:record:read` 不要说成"读取记录权限"）。话术模板：「这步要 `{标识符}` 权限，目前没开。打开飞书 → 智能伙伴设置 → 安全 → 权限管理，搜这个字符串然后启用，开完告诉我重试。**不用走开放平台审批流程**。」不要硬试调用反复失败 |
