...
智能体调用飞书功能（读多维表格、发消息、@ 人）需要对应权限。**装完一般默认够用**，但你大概率会碰到这种情况：

> 智能体说：「上传时报权限问题——应用缺少 `base:record:read` 权限」
> 或：「我没有 `bitable:app` 权限读这张表」

**只要看到 `xxx:xxx:xxx` 这种冒号分隔的小写字符串，就是缺权限的信号**。固定 4 步搞定：

### 4 步开权限

**Step 1**：复制智能体提到的权限字符串（比如 `base:record:read`）

**Step 2**：在飞书里走这条路径找到权限管理页：

> 智能体头像 → **智能伙伴设置** → 左栏 **安全** → **权限管理**

![权限管理](docs/images/feishu-permission-mgmt.png)

**Step 3**：把刚才复制的字符串粘到顶部「**搜索权限名称**」框，找到对应那一行，点右侧 **启用状态** 开关打开

**Step 4**：回对话告诉智能体「权限给你了」，它会自动重试刚才的操作（**不用重做之前的事**）

> ⚡ **不要被链接误导**：智能体有时会给你一个「飞书开放平台」的申请链接，那个走的是审批流程，慢且复杂。**直接在 智能伙伴设置 → 权限管理 里搜+开就行**，秒生效。
...
| 权限标识 | 用途 |
|---|---|
| `bitable:app` | 读写多维表格的应用元信息 |
| `bitable:app:readonly` | 只读多维表格元信息 |
| **`base:record:read`** | **读取 base 记录**（上传图片、查记录都要这个） |
| **`base:record:write`** | **写入 base 记录**（改字段、加镜头、改排期都要这个） |
| `base:table:read` / `base:table:write` | 读 / 写表结构 |
| `im:message:send_as_bot` | 智能体身份发飞书消息（私信负责人） |
| `im:message` | 处理消息（查询历史、读消息） |
| `contact:user.base:readonly` | 读飞书联系人信息（用于 @ 人员） |
| `application:application:self_manage` | 管理智能体自身资源（一般默认开） |

**用得最多的两条是 `base:record:read` 和 `base:record:write`**——遇到啥都先把这两个开了试试。

### 看不懂权限标识？

**直接把报错里那一串原样复制粘贴搜，不要自己翻译**。飞书的权限名和报错标识符是**逐字对应**的，搜中文「读取记录」反而搜不到。

### 按钮灰色点不动？

企业版飞书有些权限需要管理员二次审批 → 截屏发给你们 IT 同事让 ta 在管理后台审批一下即可。
